var Promise, _, cancelSubscription, co, getStripeCustomers, logger, processCustomers, run, stripe, dryRun, immediateCancel;
...
cancelSubscription = co.wrap(function*(sub) {
    ...
    if (!dryRun) {
        if(immediateCancel) {
            result = yield stripe.subscriptions.del(sub.id);
        }
        else {
            result = yield stripe.subscriptions.update(sub.id, {cancel_at_period_end: true});
        }
    } else {
...
run = function*(opts) {
    ...
    dryRun = opts.dryRun;
    immediateCancel = opts.immediateCancel;
